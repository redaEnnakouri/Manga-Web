<template>
    <div class="container">
        ...
        <div class="row my-2">
            <template v-if="status == 'loading'"><placeholder component="artist" classes="col-6 col-sm-4 col-md-3 col-lg-2 p-3" v-for="i in new Array(18)" :key="i"/></template>
            <div class="text-center py-5 w-100" v-else-if="status == 'error'">{{ $t('app.something_went_wrong') }}</div>
            <div v-else v-for="(artist,index) in artists" :key="index" class="col-6 col-sm-4 col-md-3 col-lg-2 p-3"><artist :artist="artist"></artist></div>
        </div>
