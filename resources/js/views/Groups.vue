<template>
    <div class="container">
        <div class="box filters d-flex justify-content-between">
            <h6 class="align-self-center my-0 mx-1"><users-icon/> {{ $t('app.groups') }}</h6>
            <div>
        ...
        <div class="row my-2">
            <template v-if="status == 'loading'"><placeholder component="group" classes="col-md-3 p-3" v-for="i in new Array(24)" :key="i"/></template>
            <div class="text-center py-5 w-100" v-else-if="status == 'error'">{{ $t('app.something_went_wrong') }}</div>
            <group v-else v-for="(group,index) in groups" :key="index" :group="group"></group>
        </div>
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.get();
            this.$meta({
                title: this.$t('meta.title.groups'),
        ...
        methods: {
            get() {
                this.status = 'loading';
                this.$api.get('groups?page=' + this.pagination.current_page, { params: this.filters }).then(response => {
