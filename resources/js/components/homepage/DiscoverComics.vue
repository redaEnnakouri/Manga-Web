<template>
    <div>
        <div v-if="status == 'done'" id="discover-comics-slider" class="box clearfix d-flex flex-column w-100 mb-3">
            <div class="box-title mx-1">
                <h6 class="float-left font-weight-bolder my-1"><compass-icon/> {{ $t('app.homepage.discover_comics') }}</h6>
        ...
        <placeholder v-else component="discover-comics" classes="mb-3"/>
    </div>
</template>
<script>
        ...
        methods: {
            loadSlider() {
                if (this.comics.length > 0) {
                    this.slider = new Glide('#discover-comics-slider', {
                        startAt: 0,
                        perView: 6,
